...
class MetaAugDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Apply
        x = self.composed(x)
        if self.mdlParams.get('with_meta', False) and self.indSet == 'trainInd':
            if torch.rand(1) < self.mdlParams['drop_augment']:
                if 'age' in self.mdlParams['meta_types']:
                    x_meta[0:18] = np.zeros([18])
            if torch.rand(1) < self.mdlParams['drop_augment']:
...
class OriginDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Apply
        if self.indSet == 'valInd':
            x = self.valid_composed(x)
            return (x, meta_vector), y, idx
        else:
            if idx % 2 == 0:
                x = self.valid_composed(x)
            else:
                x = self.train_composed(x)
            return (x, meta_vector), y, idx
